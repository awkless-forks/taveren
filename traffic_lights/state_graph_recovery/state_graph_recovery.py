...
class StateGraphRecoveryAnalysis(Analysis):
    ...
    def __init__(self, func: 'Function', fields: 'AbstractStateFields', software: str,
                 ...
                 arg0_addr: Optional[int]=0xc0000000):
        self.func = func
        self.fields = fields
    ...
    def traverse(self):
        ...
        while state_queue:
            ...
            transition = (prev_prev_abs, prev_abs_state, abs_state, time_delta) if self._temp_addr is None else (prev_prev_abs, prev_abs_state, abs_state, time_delta, temp_delta)
            ...
            for delta in time_deltas:
                for constraint in next_state.solver.constraints:
                    ...
                    if delta.args[0] in constraint.variables:
                        if self._temp_addr is not None:
                            simplified_constraint = claripy.simplify(constraint)
                            if simplified_constraint is not None:
                                constraint = simplified_constraint
                    ...
                    if constraint.op == "__eq__" and constraint.args[0] is delta:
                        continue
                    elif constraint.op in ('ULE'):
                        if constraint.args[0].args[1] is delta:
                            if constraint.args[1].op == 'BVV':    # simulink arm32
                                ...
                                if step != 0 and step < 255:
                                    ...
                                    continue
                            if constraint.args[1].args[0].op == 'BVV':   # arduino arm32
    ...
    def _symbolize_temp(self, state: 'SimState') -> Dict[str, claripy.ast.Base]:
        ...
        state.preconstrainer.preconstrain(claripy.BVV(prev_temp, 64).raw_to_fp(), self._temperature)
    ...
    def _symbolically_advance_temp(self, state: 'SimState') -> List[claripy.ast.Bits]:
        ...
        state.preconstrainer.preconstrain(claripy.FPV(0.5, claripy.fp.FSORT_DOUBLE), temp_delta)
    ...
    def _initialize_state(self, init_state=None) -> 'SimState':
        if init_state is not None:
            ...
            if self.project.arch.name == 'AMD64':
                s.regs.rdi = self.arg0_addr
            elif 'ARM' in self.project.arch.name:
                s.regs.r0 = self.arg0_addr
        else:
